...
function generateDifficultiesList() {
    let checkBoxList = '<div> <label>Difficulties:</label></div>';
    const difficultDivTemplate =
        '<div> <input type="checkbox" id="{{data}}" checked/> <label> {{data}} </label> </div>';
    for (let i = 0; i < difficulties.length; i++) {
        checkBoxList += renderTemplate(
            difficultDivTemplate,
            difficulties[i].name
        );
    }
    document
        .getElementById('thirdColumnContainer')
        .insertAdjacentHTML('afterbegin', checkBoxList);
}

function randomizeTowerNumber() {
    var towersTot = 0;
    do {
        var primaryTowerNumber = Math.abs(
            Math.floor(Math.random() * towers.primary.length - 2)
        );
        var militaryTowerNumber = Math.abs(
            Math.floor(Math.random() * towers.military.length - 1)
        );
        var magicTowerNumber = Math.abs(
            Math.floor(Math.random() * towers.magic.length - 1)
        );
        var supportTowerNumber = Math.abs(
            Math.floor(Math.random() * towers.support.length - 1)
        );
        towersTot =
            supportTowerNumber +
            magicTowerNumber +
...
function showChallenge(hero, map, towers, mode, extra) {
    if (!map) {
        window.alert('You have to choose at least one map difficult!');
        return;
    }
    if (!mode) {
        window.alert('You have to choose at least one mode!');
        return;
    }
    if (!towers) {
        window.alert('You have to choose at least one tower!');
        return;
    }
    ...
    display.insertAdjacentHTML(
        'beforeend',
        '<div> NOTE: Challenge may not be possible </div><div id="imgDiv"></div>'
    );
    document
        .getElementById('imgDiv')
        .insertAdjacentHTML(
            'beforeend',
            '<img class ="modeImg"src="img/mode/' + mode + '.png" />'
        );
    document
        .getElementById('imgDiv')
        .insertAdjacentHTML(
            'beforeend',
            '<img class ="mapImg" src="img/maps/' + map + '.png" />'
        );
    if (extra) {
        display.insertAdjacentHTML(
            'beforeend',
        ...
        if (extra == '#FindACoolName') {
            var explanation =
                'Upgrade the first tower you place using top path, the second one using middle path, the third one using bottom path, the fourth one again using top path and so on... <br>' +
    ...
    if (hero) {
        document
            ...
            .insertAdjacentHTML(
                'beforeend',
                '<img class ="loadOutImg" src="img/heroes/' + hero + '.png" />'
            );
    }
    ...
    for (key in towers) {
        document
            ...
            .insertAdjacentHTML(
                'beforeend',
                '<img class="loadOutImg" src="img/monkeys/' +
                    towers[key] +
                    '.png" />'
            );
        //towersDiv += '<p>'++'</p>';
    ...
    switch (mode) {
        ...
        default:
            ...
            for (var x = 0; x < towerTypes.length; x++) {
                for (
                    var y = 0;
                    y < document.getElementById(towerTypes[x]).value;
                    y++
                ) {
                    do {
                        randomTower =
                            towers[towerTypes[x]][
                                Math.floor(
                                    Math.random() * towers[towerTypes[x]].length
                                )
                            ].name;
...
function randomizeExtraRule() {
    if (!document.getElementById('extraRule').checked) {
        return null;
    }
    let extraRuleList = [
        'Use top path only',
        'Use middle path only',
        'Use bottom path only',
        '#FindACoolName',
        'No 4th tier upgrade',
        'Use max 2 towers',
    ];
